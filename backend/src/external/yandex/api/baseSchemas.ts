...
export const stationSchema = z.object({
  ...
  transport_type: z.enum(["plane", "train", "suburban", "bus", "water", "helicopter"]).describe("Type of transport"),
  station_type: z
    .enum([
      "station",
      "platform",
...
export const carrierSchema = z.object({
  ...
  thread_method_link: z.string().optional().describe("URL for thread info request"),
})

export const transportSubtypeSchema = z.object({
  color: z.string().nullable().describe("Main color of transport in hex format"),
...
export const threadSchema = z.object({
  ...
  express_type: z.enum(["express", "aeroexpress"]).nullable().describe("Express train type"),
})
...
export const threadStopSchema = z.object({
  ...
  departure: z.string().nullable().describe("Departure time in ISO 8601 format"),
...
export const stationListItemSchema = z
  .object({
    ...
    longitude: z.union([z.number(), z.literal("")]).describe("Station longitude"),
    latitude: z.union([z.number(), z.literal("")]).describe("Station latitude"),
  })
  .describe('If a station has latitude or longitude equal to "", it is invalid')
...
export const settlementSchema = z.object({
  ...
  stations: z
    ...
    .transform((arr) => arr.filter((s) => s.latitude !== "" && s.longitude !== ""))
...
export const countrySchema = z
  .object({
    ...
    codes: z.object({
      yandex_code: z.string().optional().describe("Country code in Yandex Schedule system"),
    }),
...
export type ThreadWithIntervalContract = z.infer<typeof threadSchemaWithInterval>
