...
import { paginationSchema, directionSchema, threadSchemaWithInterval, stationSchema } from "../baseSchemas"
...
export const stationScheduleParamsSchema = z.object({
  ...
  system: z.enum(["yandex", "iata", "sirena", "express", "esr"]).optional().describe("Station code system"),
  show_systems: z.enum(["yandex", "esr", "all"]).optional().describe("Response code systems"),
...
export const stationScheduleResponseSchema = z.object({
  ...
  schedule_direction: directionSchema.optional().describe("Requested direction info, if specified"),
  directions: z.array(directionSchema).optional().describe("Available directions for suburban trains"),
})

export type StationScheduleResponse = z.infer<typeof stationScheduleResponseSchema>
 ...
 * \`\`\`
 */
export const fetchStationSchedule = async (params: StationScheduleParams): Promise<Result<StationScheduleResponse>> => {
  return makeYandexApiRequest(SCHEDULE_ENDPOINT, stationScheduleResponseSchema, params)
}
